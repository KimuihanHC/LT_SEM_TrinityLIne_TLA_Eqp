...
#define WM_COMM_TXBUF				WM_USER+10	// A character was received and placed in the input buffer. 


class CSerialPort
{														 
public:
	BOOL IsConnected(void)	{	return m_bThreadAlive;	};
	BOOL ClosePort(void);
	void WriteToPort(LPCSTR str);
	...
	void		SetOwnerWnd(__in HWND pOwner)
	{
		m_pOwner = pOwner;
	}
	...
	void		WriteToPort(char* pbytData, int nSize);		//string);
