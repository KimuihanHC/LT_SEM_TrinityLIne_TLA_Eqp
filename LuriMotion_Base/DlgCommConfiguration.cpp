...
#if (20230905)
	DDX_Control(pDX, IDC_STC_LAN_CONNECT_COMMON1, m_stComponent_Lan_Common_Sub[IP_COMMON_SCREW]);
	DDX_Control(pDX, IDC_STC_CONNECT_LAN_IP_COMMON1, m_stComponent_Lan_Common_Ip[IP_COMMON_SCREW]);
	DDX_Control(pDX, IDC_BTN_LAN_CONNECT_COMMON1, m_btnComponent_Lan_Common_Connect[IP_COMMON_SCREW]);
	DDX_Control(pDX, IDC_BTN_LAN_DISCONNECT_COMMON1, m_btnComponent_Lan_Common_DisConnect[IP_COMMON_SCREW]);
	DDX_Control(pDX, IDC_IPADDRESS_LAN_COMMON1, m_lanIpCommonCtrl[IP_COMMON_SCREW]);
	DDX_Control(pDX, IDC_EDIT_LAN_PORT_INPUT_COMMON1, m_edComponent_Lan_Common_Port[IP_COMMON_SCREW]);
#endif
...
// #endif
ON_BN_CLICKED(IDC_BTN_LAN_SUB_COMMON1, &CDlgCommConfiguration::OnBnClickedBtnLanSubCommon1)
ON_BN_CLICKED(IDC_BTN_LAN_SUB_COMMON2, &CDlgCommConfiguration::OnBnClickedBtnLanSubCommon2)
END_MESSAGE_MAP()
...
void CDlgCommConfiguration::OnBnClickedBtnLanConnectCommon1()
{
	...
	if (m_TCPCommon[0].m_bConnected)
	{
		//m_UdpTester[0].Connect_Tester(stTester.dwTesterIP[0], stTester.nTesterPort[0], stTester.strTesterConnect[0], 1);
		m_TCPCommon[IP_COMMON_SCREW].Send_Common_Communication(eScrew_Communication_Start);
	}
	else
	{
		strTemp = "192.168.0.2";
		...
		m_TCPCommon[0].Connect_Common(strTemp, nTesterPort);

		COMMON->DoEvent(100);
		m_TCPCommon[IP_COMMON_SCREW].InitVariable(IP_COMMON_SCREW);
		m_TCPCommon[IP_COMMON_SCREW].Send_Common_Communication(eScrew_Communication_Start);

		m_TCPCommon[IP_COMMON_SCREW].SetOwnerHWND(this->m_hWnd);
	}
}
...
{
	UpdateData(TRUE);
	m_TCPCommon[IP_COMMON_SCREW].Send_Common_Communication(eScrew_Communication_Stop);
	m_TCPCommon[IP_COMMON_SCREW].CloseSocket();
}
#endif

void CDlgCommConfiguration::OnBnClickedBtnLanSubCommon1()
{
	// TODO: Add your control notification handler code here
	if (m_TCPCommon[0].m_bConnected)
	  m_TCPCommon[IP_COMMON_SCREW].Send_Common_Communication(eScrew_DataMessageSubscribe, 15);
}


void CDlgCommConfiguration::OnBnClickedBtnLanSubCommon2()
{
	// TODO: Add your control notification handler code here
	if (m_TCPCommon[0].m_bConnected)
		m_TCPCommon[IP_COMMON_SCREW].Send_Common_Communication(eScrew_DataMessageSubscribe, 1201);
}
