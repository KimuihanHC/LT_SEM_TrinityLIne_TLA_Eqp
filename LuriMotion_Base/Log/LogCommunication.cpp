...
CLogCommunication::CLogCommunication( LPCTSTR lpszServiceName ) :
...
CLogCommunication::~CLogCommunication()
{
	if (m_bStart)
		Stop();
}

void CLogCommunication::GetLogPathName( CTime& Time, CExString& strLogPathName )
{
...
void CLogCommunication::GetCSVPathName( CTime& Time, CExString& strCSVPathName )
{
	int nYear = Time.GetYear();
	int nMonth = Time.GetMonth();
	int nDay = Time.GetDay();
...
void CLogCommunication::UseCSVFile( BOOL bUse, LPCTSTR lpszCSVHeader )
{
	m_bUseCSVFile = bUse;
	m_strCSVHeader.Format( _T("%s\r\n"), lpszCSVHeader );
}

void CLogCommunication::UseTextFile( BOOL bUse )
{
	m_bUseTxtFile = bUse;
}

void CLogCommunication::UseMonthlySplitFolder( BOOL bUse )
{
	m_bMonthlySplitFolder = bUse;
}

BOOL CLogCommunication::Start()
{
...
BOOL CLogCommunication::OpenLogFile( CExString& strLogPathName )
{
	if ( !m_bUseTxtFile )
		return TRUE;
	...
	if ( !m_File.Open(strLogPathName, CFile::modeRead|CFile::shareDenyWrite) )
	{
		...
		strComment.Format( _T("#################### Luritech Communication Log ####################\r\n")
...
BOOL CLogCommunication::OpenCSVFile( CExString& strCSVPathName )
{
	if ( !m_bUseCSVFile )
		return TRUE;
...
void CLogCommunication::LogOut( LPCTSTR lpszId, LPCTSTR lpszFormat, ... )
{
...
void CLogCommunication::DeleteExpiredCSVFile()
{
...
void CLogCommunication::DeleteExpiredLogFile()
{
...
void CLogCommunication::CreateFolder(LPCTSTR lpszPath)
{
